(function(){
  ...
  function createAudioGraph(){
    ...
    try{ sourceNode = audioCtx.createMediaElementSource(audioEl); }catch(e){ sourceNode = null; }
    ...
    if (sourceNode){ sourceNode.connect(bassNode); bassNode.connect(trebleNode); trebleNode.connect(gainNode); gainNode.connect(analyserNode); analyserNode.connect(audioCtx.destination); }
  ...
  playBtn.addEventListener('click', async ()=>{ if(!audioEl.src){ alert('Add audio files first'); return; } if(audioEl.paused) await playAudio(); else pauseAudio(); });
  ...
  document.getElementById('saveCustomBtn').addEventListener('click', ()=>{ const preset={gain:+gainSlider.value,bass:+bassSlider.value,treble:+treble.value}; localStorage.setItem('mcm_custom_preset',JSON.stringify(preset)); alert('Custom preset saved'); });
  ...
  window.addEventListener('appinstalled', ()=>{ thankYou.style.display='block'; setTimeout(()=> thankYou.style.display='none',3500); });
